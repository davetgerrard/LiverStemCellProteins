...
## this is an attempt to create a re-usable function to run the topGO score tests over a set of PCs.
...
#pcaScores <- cbind(subset(stemcommondata, select=c("Name","Uniprot.Id", "Uniprot.Accession")) ,stem.princomp.cor$scores)


#geneList <- pcaScores$Comp.1			#take an aribitrary set of scores.
#names(geneList) <- pcaScores$Uniprot.Accession	# the names are important.


scoreGoOnPcaScores <- function(pcaScores, geneNames, description="test",elimCutOff = 0.01, pcs = 3, 
					goGraphs = c("BP","MF","CC"), nodeSizeValue = 10, topNodesValue = 20, 
					topTerms = 50, go2prot)  {


	#initialise geneList from #Comp.1
	geneList <- pcaScores[,1]			#take an aribitrary set of scores.
	names(geneList) <- geneNames			# the names are important.
		

	goDataCollection.scored <- list()
	for(thisGOgraph in goGraphs )  {
		goDataCollection.scored[[thisGOgraph]] <- new("topGOdata",
	...
	for( i in 1:length(pcs))  {
		...
		geneList <- pcaScores[,compHead]	# get PC scores for proteins
		names(geneList) <- geneNames		# make sure scores have names of proteins
		...
		for(thisGOgraph in goGraphs )  {		# loop through 3 GO ontologies
			...
			goDataCollection.scored[[thisGOgraph]] <- updateGenes(goDataCollection.scored[[thisGOgraph]],geneList,topDiffGenes)		
			#INFO: runScoreGoTests() performs the tests and returns an ouptut table
			summaryScoreResults[[i]] <- rbind(summaryScoreResults[[i]],runScoreGoTests(goDataCollection.scored[[thisGOgraph]],geneList,topDiffGenes))
	...
	for(i in 1:length(summaryScoreResults)) {
		outFileName <- paste(description,"GoSummaryByPc",i,"tab", sep=".")
		write.table(summaryScoreResults[[i]],file=paste("output",outFileName,sep="/"),sep="\t",quote=F,row.names=F) 
		rm(outFileName)
	}
	
	return(summaryScoreResults)
}
stopifnot(FALSE)

H9.scoreGoOnPcaScores <- scoreGoOnPcaScores(pcaScores=H9.noOutliers.pca$scores, geneNames=H9.complete.noOutliers$Uniprot.Accession, 
					description="H9.comp.noOutliers", go2prot=go2prot)

H7.scoreGoOnPcaScores <- scoreGoOnPcaScores(pcaScores=H7.noOutliers.pca$scores, geneNames=H7.complete.noOutliers$Uniprot.Accession, 
					description="H7.comp.noOutliers", go2prot=go2prot)
...
for(i in 1:length(summaryScoreResults)) {
	...
	summaryScoreResults[[i]]$bestCluster  <- apply(summaryScoreResults[[i]],1, FUN= function(x) listBestOverlappingCluster(
						goProteins.valid=intersect(listProtsInGoFromList(goTerm=x["goTerm"],ontology=x["ontology"],goDataCollection.scored),validProts),
