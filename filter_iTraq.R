...
H7rawdata<-read.csv(file="H7StemiTRAQ.csv") 
...
H7data$Accession <- matrix(unlist(strsplit(as.character(H7data$Accession),"|",fixed=T)),ncol=3,byrow=T)[,2]

write.csv(H7data,file="H7data.csv")
############################################


H9rawdata<-read.csv(file="H9StemiTRAQ.csv") 
...
H9data$Accession <- matrix(unlist(strsplit(as.character(H9data$Accession),"|",fixed=T)),ncol=3,byrow=T)[,2]

write.csv(H9data,file="H9data.csv")